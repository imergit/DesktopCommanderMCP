...
const hostExe = 'C:/ProgramData/ImerTerm/bin/ImerTerm.Host.exe';
const localRoot = 'C:/ProgramData/ImerTerm';
...
try {
  ...
  assert.equal(caps.capabilities?.source_id, 'local-controller');
  assert.deepEqual(caps.capabilities?.openssh_targets, ['gb10','ufv-it01']);
  assert.deepEqual(caps.capabilities?.routeros_targets, ['crs112-router-223']);
  console.log('PASS ImerTerm capabilities through MCP adapter');
  const workingDirectory = 'D:/019.imerterm';
  ...
  const sshDenied = await shadow.callTool({ name: 'imerterm_run_ssh', arguments: {
    project_id: 'imerterm', target_id: '__m8-invalid-ssh__', mutation_class: 'NONE', run_as: 'USER', script: 'true'
  } });
  assert.equal(sshDenied.isError, true, 'disabled OpenSSH target must fail closed');
  assert.match(text(sshDenied), /does not expose target __m8-invalid-ssh__/);
  const rosDenied = await shadow.callTool({ name: 'imerterm_run_routeros', arguments: {
    project_id: 'imerterm', target_id: '__m8-invalid-ros__', mutation_class: 'NONE', command: '/system identity print'
  } });
  assert.equal(rosDenied.isError, true, 'disabled RouterOS target must fail closed');
  assert.match(text(rosDenied), /does not expose target __m8-invalid-ros__/);
